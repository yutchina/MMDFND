...
class MultiDomainPLEFENDModel(torch.nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        fusion_gate_input0 = fusion_share_feature
...
##continue
        ...
        for i in range(self.domain_num):
            ...
            final_gate_input = fusion_tempfeat_main_task
        ...
        for epoch in range(self.epoches):
            ...
            if mark == 'save':
                torch.save(self.model.state_dict(),
                           os.path.join(self.save_param_dir, 'parameter_clipPLED.pkl'))
            elif mark == 'esc':
                break
            else:
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_param_dir, 'parameter_clipPLED.pkl')))
        results0,results1,results2,results3 = self.test(self.test_loader)

        return results0, os.path.join(self.save_param_dir, 'parameter_clipPLED.pkl')
