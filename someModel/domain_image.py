...
class MultiDomainPLEFENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, out_channels, dropout):
        ...
        for i in range(self.num_share):
            ...
            text_share = []
            for j in range(self.num_expert):
                image_share.append(cnn_extractor(self.text_dim,feature_kernel))
    ...
    def forward(self, **kwargs):
        ...
        for i in range(self.domain_num):
            ...
            pre = torch.sigmoid(self.image_classifier_list[i](image_gate_expert_value[i]).squeeze(1))
        ...
        label_pred_list = torch.cat((label_pred_list[0], label_pred_list[1], label_pred_list[2], label_pred_list[3],
                                     label_pred_list[4], label_pred_list[5], label_pred_list[6], label_pred_list[7]))
