...
class MultiDomainPLEFENDModel(torch.nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        self.n_node = image_feature.size()[0]
        ...
        # 将 domain_star_emb_list 及其子模块移到GPU上
        self.domain_star_emb_list.cuda()

        # 遍历 domain_star_emb_list，将其中的每个子模块移到GPU上
        for star_emb_list in self.domain_star_emb_list:
            star_emb_list.cuda()
            for domain_star_emb in star_emb_list:
                domain_star_emb.cuda()
...
##continue
        ...
        domain_special_list = []
        for i in range(self.domain_num):
            dom_star_emb1 = self.domain_star_emb_list[i][0].weight
            dom_star_emb2 = self.domain_star_emb_list[i][1].weight
            dom_star_emb3 = self.domain_star_emb_list[i][2].weight
            dom_star_emb4 = self.domain_star_emb_list[i][3].weight
