...
class MultiDomainPLEFENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, out_channels, dropout):
        ...
        domain_star_emb_list = torch.nn.ModuleList()
        for i in range(self.domain_num):
            star_emb_list = torch.nn.ModuleList()
            self.domain_star_emb1 = nn.Embedding(self.n_node, self.feature_emb_size)
            self.domain_star_emb2 = nn.Embedding(self.n_node, self.feature_emb_size)
            self.domain_star_emb3 = nn.Embedding(self.n_node, self.feature_emb_size)
            self.domain_star_emb4 = nn.Embedding(self.n_node, self.feature_emb_size)
            #torch.nn.init.normal_(self.domain_star_emb1.weight, 0, 0.02)
            #torch.nn.init.normal_(self.domain_star_emb2.weight, 0, 0.02)
            #torch.nn.init.normal_(self.domain_star_emb3.weight, 0, 0.02)
            #torch.nn.init.normal_(self.domain_star_emb4.weight, 0, 0.02)
            star_emb_list.append(self.domain_star_emb1)
            star_emb_list.append(self.domain_star_emb2)
            star_emb_list.append(self.domain_star_emb3)
            star_emb_list.append(self.domain_star_emb4)
            domain_star_emb_list.append(star_emb_list)
        self.domain_star_emb_list = domain_star_emb_list
        self.active = nn.SiLU()
    ...
    def fusion_img_text(self, image_emb, text_emb,fusion_emb,star_emb1, star_emb2, star_emb3, star_emb4,mlp_img,mlp_text,mlp_fusion,transformers,mlp_star_f1,mlp_star_f2):
    ...
    def fusion_img_text2(self, image_emb, text_emb,fusion_emb,star_emb1, star_emb2, star_emb3, star_emb4,mlp_img,mlp_text,mlp_fusion,transformers,mlp_star_f1,mlp_star_f2):
        for img_feature_num in range(0, self.feature_num):
            if img_feature_num == 0:
                img_feature_seq = mlp_img[img_feature_num](image_emb)
                img_feature_seq = img_feature_seq.unsqueeze(1)
    ...
    def forward(self, **kwargs):
        ...
        #fusion_share_feature = torch.cat((clip_fusion_feature,text_gate_share_expert_value, image_gate_share_expert_value), dim=-1)
...
##continue
        ...
        #aaaaaaaa
        # pivot fusion
        share_star_emb1 = self.star_emb1.weight
        share_star_emb2 = self.star_emb2.weight
        share_star_emb3 = self.star_emb3.weight
        share_star_emb4 = self.star_emb4.weight
        text_gate_share_expert_value = text_gate_share_expert_value[0]
        image_gate_share_expert_value = image_gate_share_expert_value[0]
        fusion_gate_share_expert_value = fusion_gate_share_expert_value0[0]
        cross_knowledge = self.fusion_img_text(image_gate_share_expert_value, text_gate_share_expert_value, fusion_gate_share_expert_value, share_star_emb1, share_star_emb2, share_star_emb3, share_star_emb4,self.mlp_img,self.mlp_text,self.pivot_mlp_fusion,self.transformers,self.mlp_star_f1,self.mlp_star_f2)
        #aaaaaaaaa
        domain_special_list = []
        for i in range(self.domain_num):
            dom_star_emb1 = self.domain_star_emb_list[i][0].weight.cuda()
            dom_star_emb2 = self.domain_star_emb_list[i][1].weight.cuda()
            dom_star_emb3 = self.domain_star_emb_list[i][2].weight.cuda()
            dom_star_emb4 = self.domain_star_emb_list[i][3].weight.cuda()
            ...
            domain_knowledge = self.fusion_img_text(image_spacial_knowledge, text_spacial_knowledge,
                                                   fusion_spacial_knowledge, dom_star_emb1, dom_star_emb2,
                                                   dom_star_emb3, dom_star_emb4,self.mlp_img_list[i],self.mlp_text_list[i],self.pivot_mlp_fusion_list[i],self.transformers_list[i],self.mlp_star_f1_list[i],self.mlp_star_f2_list[i])
