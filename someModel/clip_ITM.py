    ...
    def train(self):
        ...
        for epoch in range(self.epoches):
            ...
            for step_n, batch in enumerate(train_data_iter):
                ...
                loss = loss0+0.15*loss1+0.15*loss2+0.15*loss3
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            if mark == 'save':
                torch.save(self.model.state_dict(),
                           os.path.join(self.save_param_dir, 'parameter_clipITM.pkl'))
            elif mark == 'esc':
                break
            else:
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_param_dir, 'parameter_clipITM.pkl')))
        results0,results1,results2,results3 = self.test(self.test_loader)

        return results0, os.path.join(self.save_param_dir, 'parameter_clipITM.pkl')
