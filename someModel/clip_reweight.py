...
##continue
        ...
        concat_feature_list = []
        for i in range(self.domain_num):
            final_image_feature0 = final_image_feature[i]  # shared_image_feature * (image_atn_score)
            final_text_feature0 = final_text_feature[i]  # shared_text_feature * (text_atn_score)
        ...
        for epoch in range(self.epoches):
            ...
            if mark == 'save':
                torch.save(self.model.state_dict(),
                           os.path.join(self.save_param_dir, 'parameter_clipre.pkl'))
            elif mark == 'esc':
                break
            else:
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_param_dir, 'parameter_clipre.pkl')))
        results0,results1,results2,results3 = self.test(self.test_loader)

        return results0, os.path.join(self.save_param_dir, 'parameter_clipre.pkl')
