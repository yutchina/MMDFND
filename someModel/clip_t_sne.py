...
class MultiDomainPLEFENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, out_channels, dropout):
        ...
        self.cc_classifier = MLP(320, mlp_dims, dropout)
    ...
    def forward(self, **kwargs):
        ...
        fusion_share_feature = torch.cat(
            (clip_fusion_feature, text_gate_share_expert_value, image_gate_share_expert_value), dim=-1)
        ...
        text_label_pred_list = torch.cat(
            (text_label_pred_list[0], text_label_pred_list[1], text_label_pred_list[2], text_label_pred_list[3],
             text_label_pred_list[4], text_label_pred_list[5], text_label_pred_list[6], text_label_pred_list[7],
             text_label_pred_list[8]))
        # image
        image_only_output = []
        image_label_pred = []
        final_image_feature = []
        ...
        image_label_pred_list = torch.cat(
            (image_label_pred_list[0], image_label_pred_list[1], image_label_pred_list[2], image_label_pred_list[3],
             image_label_pred_list[4], image_label_pred_list[5], image_label_pred_list[6], image_label_pred_list[7],
             image_label_pred_list[8]))
        ...
        for i in range(self.domain_num):
            ...
            fusion_mu.append(
                self.mapping_CC_MLP_mu(fusion_atn_score[i].clone().detach().view(-1, 1)))  # 1-aux_atn_score
            image_sigma.append(
                self.mapping_IS_MLP_sigma(torch.sigmoid(image_only_output[i]).clone().detach().view(-1, 1)))
            text_sigma.append(self.mapping_T_MLP_sigma(torch.sigmoid(text_only_output[i]).clone().detach().view(-1, 1)))
            fusion_sigma.append(
                self.mapping_CC_MLP_sigma(fusion_atn_score[i].clone().detach().view(-1, 1)))  # 1-aux_atn_score

        concat_feature_list = []
        for i in range(self.domain_num):
            final_image_feature0 = self.adaIN(final_image_feature[i], image_mu[i],
                                              image_sigma[i])  # shared_image_feature * (image_atn_score)
            ...
            irr_score = torch.ones_like(final_fusion_feature0) * self.irrelevant_tensor[
                i].cuda()  # torch.ones_like(shared_mm_feature).cuda()
        ...
        final_label_pred_list = torch.cat(
            (final_label_pred_list[0], final_label_pred_list[1], final_label_pred_list[2], final_label_pred_list[3],
             final_label_pred_list[4], final_label_pred_list[5], final_label_pred_list[6], final_label_pred_list[7],
             final_label_pred_list[8]))

        cc_irrelevant_token = torch.sigmoid(self.cc_classifier(irrelevant_token)).squeeze(-1)
        ...
        for i in range(self.domain_num):
            # label_pred.append(torch.sigmoid(self.text_classifier_list[i](text_gate_expert_value[i]).squeeze(1)))
            pre = final_gate_expert_value[i]
            tsne_label_pred.append(pre)
        tsne_label_pred_list = []
        for i in range(self.domain_num):
            tsne_label_pred_list.append(tsne_label_pred[i][idxs.squeeze() == i])
        #tsne_label_pred_list = torch.cat(
         #   (tsne_label_pred_list[0], tsne_label_pred_list[1], tsne_label_pred_list[2], tsne_label_pred_list[3],
          #   tsne_label_pred_list[4], tsne_label_pred_list[5], tsne_label_pred_list[6], tsne_label_pred_list[7],
           #  tsne_label_pred_list[8]))
        ...
        return tsne_label_pred_list,cc_irrelevant_token, final_label_pred_list, fusion_label_pred_list, image_label_pred_list, text_label_pred_list
        ...
        for epoch in range(self.epoches):
            ...
            for step_n, batch in enumerate(train_data_iter):
                ...
                batch_label = torch.cat((label[idxs.squeeze() == 0], label[idxs.squeeze() == 1],
                                         ...
                                         label[idxs.squeeze() == 8]))

                tsnefeature,cc, final_label_pred_list, fusion_label_pred_list, image_label_pred_list, text_label_pred_list = self.model(
                    **batch_data)
                ...
                loss3 = loss_fn(text_label_pred_list, label.float())
                loss_cc = loss_fn(cc, batch_label.float())
                loss = loss0 + 0.2 * loss1 + 0.2 * loss2 + 0.2 * loss3 + 0.15 * loss_cc
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
            ...
            self.tsne(self.train_loader)
            mark = recorder.add(results0)
            if mark == 'save':
                torch.save(self.model.state_dict(),
                           os.path.join(self.save_param_dir, 'parameter_clip9.pkl'))
            elif mark == 'esc':
                break
            else:
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_param_dir, 'parameter_clip9.pkl')))
        results0, results1, results2, results3 = self.test(self.test_loader)
        #self.tsne(self.val_loader)
        #self.tsne(self.test_loader)
        return results0, os.path.join(self.save_param_dir, 'parameter_clip9.pkl')
    ...
    def test(self, dataloader):
        ...
        for step_n, batch in enumerate(data_iter):
            with torch.no_grad():
                ...
                _,_, final_label_pred_list, fusion_label_pred_list, image_label_pred_list, text_label_pred_list = self.model(
                    **batch_data)
        ...
        return metricsTrueFalse(label1, pred0, category, self.category_dict), metricsTrueFalse(label1, pred1, category,
                                                                                               self.category_dict), metricsTrueFalse(
            label1, pred2, category, self.category_dict), metricsTrueFalse(label1, pred3, category, self.category_dict)
    ...
    def tsne(self, dataloader):
        ...
        for step_n, batch in enumerate(data_iter):
            with torch.no_grad():
                ...
                label = batch_data['label']
                batch_category = batch_data['category']
                tsnefeature,_, _, _, _, _ = self.model(
                    **batch_data)
                ...
                batch_label = torch.cat((label[idxs.squeeze() == 0], label[idxs.squeeze() == 1],
                                         ...
                                         label[idxs.squeeze() == 6], label[idxs.squeeze() == 7],
                                         label[idxs.squeeze() == 8]))
                batch_label0.append(label[idxs.squeeze() == 0].detach().cpu())
                batch_label1.append(label[idxs.squeeze() == 1].detach().cpu())
                batch_label2.append(label[idxs.squeeze() == 2].detach().cpu())
                batch_label3.append(label[idxs.squeeze() == 3].detach().cpu())
                batch_label4.append(label[idxs.squeeze() == 4].detach().cpu())
                batch_label5.append(label[idxs.squeeze() == 5].detach().cpu())
                batch_label6.append(label[idxs.squeeze() == 6].detach().cpu())
                batch_label7.append(label[idxs.squeeze() == 7].detach().cpu())
                batch_label8.append(label[idxs.squeeze() == 8].detach().cpu())

                data0.append(tsnefeature[0].detach().cpu())
                data1.append(tsnefeature[1].detach().cpu())
                data2.append(tsnefeature[2].detach().cpu())
                data3.append(tsnefeature[3].detach().cpu())
                data4.append(tsnefeature[4].detach().cpu())
                data5.append(tsnefeature[5].detach().cpu())
                data6.append(tsnefeature[6].detach().cpu())
                data7.append(tsnefeature[7].detach().cpu())
                data8.append(tsnefeature[8].detach().cpu())
        ...
        #all_data = torch.cat(data0)
        #all_data_np = all_data.numpy()
        #all_data_np = all_data_np[:2000]
        #all_labels = torch.cat(labels)
        #all_labels = torch.cat(batch_label0)
        #all_data_labels = all_labels.numpy()
        #all_data_labels = all_data_labels[:2000]

        #0
        data0 = torch.cat(data0)
        data0 = data0.numpy()
        label0 = torch.cat(batch_label0)
        label0 = label0.numpy()

        tsne = TSNE(n_components=2, perplexity=30, early_exaggeration=12, learning_rate=200, random_state=42)
        # tsne = TSNE(n_components=2, perplexity=30, learning_rate=200, random_state=0)
        tsne_results0 = tsne.fit_transform(data0)

        # 可视化
        plt.figure(figsize=(6, 6))
        colors = ['#FFBCBB', '#B99CD4']
        for i, label in enumerate(np.unique(label0)):
            indices = np.where(label0 == label)
            plt.scatter(tsne_results0[indices, 0], tsne_results0[indices, 1], color=colors[i],
                        label=f'Class {label}')
        ...
        plt.savefig('dom0.png', dpi=300, bbox_inches='tight', pad_inches=0)
        ...
        data1 = data1.numpy()
        label1 = torch.cat(batch_label1)
        label1 = label1.numpy()

        tsne = TSNE(n_components=2, perplexity=30, early_exaggeration=12, learning_rate=200, random_state=42)
        # tsne = TSNE(n_components=2, perplexity=30, learning_rate=200, random_state=0)
        tsne_results1 = tsne.fit_transform(data1)

        # 可视化
        plt.figure(figsize=(6, 6))
        colors = ['#FFBCBB', '#B99CD4']
        for i, label in enumerate(np.unique(label1)):
            indices = np.where(label1 == label)
            plt.scatter(tsne_results1[indices, 0], tsne_results1[indices, 1], color=colors[i],
                        label=f'Class {label}')
        plt.gca().set_axis_off()
        ...
        plt.gca().xaxis.set_major_locator(plt.NullLocator())
        plt.gca().yaxis.set_major_locator(plt.NullLocator())
        plt.savefig('dom1.png', dpi=300, bbox_inches='tight', pad_inches=0)
        ...
        data2 = data2.numpy()
        label2 = torch.cat(batch_label2)
        label2 = label2.numpy()

        tsne = TSNE(n_components=2, perplexity=30, early_exaggeration=12, learning_rate=200, random_state=42)
        # tsne = TSNE(n_components=2, perplexity=30, learning_rate=200, random_state=0)
        tsne_results2 = tsne.fit_transform(data2)

        # 可视化
        plt.figure(figsize=(6, 6))
        colors = ['#FFBCBB', '#B99CD4']
        for i, label in enumerate(np.unique(label2)):
            indices = np.where(label2 == label)
            plt.scatter(tsne_results2[indices, 0], tsne_results2[indices, 1], color=colors[i],
                        label=f'Class {label}')
        plt.gca().set_axis_off()
        ...
        plt.gca().xaxis.set_major_locator(plt.NullLocator())
        plt.gca().yaxis.set_major_locator(plt.NullLocator())
        plt.savefig('dom2.png', dpi=300, bbox_inches='tight', pad_inches=0)

        # 显示图表（注意，显示出来的图表可能会因为 Jupyter Notebook 或其他IDE的显示设置而包含边距）
        plt.show()
        # 3
        data3 = torch.cat(data3)
        data3 = data3.numpy()
        label3 = torch.cat(batch_label3)
        label3 = label3.numpy()

        tsne = TSNE(n_components=2, perplexity=30, early_exaggeration=12, learning_rate=200, random_state=42)
        # tsne = TSNE(n_components=2, perplexity=30, learning_rate=200, random_state=0)
        tsne_results3 = tsne.fit_transform(data3)

        # 可视化
        plt.figure(figsize=(6, 6))
        colors = ['#FFBCBB', '#B99CD4']
        for i, label in enumerate(np.unique(label3)):
            indices = np.where(label3 == label)
            plt.scatter(tsne_results3[indices, 0], tsne_results3[indices, 1], color=colors[i],
                        label=f'Class {label}')
        plt.gca().set_axis_off()
        ...
        plt.gca().xaxis.set_major_locator(plt.NullLocator())
        plt.gca().yaxis.set_major_locator(plt.NullLocator())
        plt.savefig('dom3.png', dpi=300, bbox_inches='tight', pad_inches=0)

        # 显示图表（注意，显示出来的图表可能会因为 Jupyter Notebook 或其他IDE的显示设置而包含边距）
        plt.show()
        # 4
        data4 = torch.cat(data4)
        data4 = data4.numpy()
        label4 = torch.cat(batch_label4)
        label4 = label4.numpy()

        tsne = TSNE(n_components=2, perplexity=30, early_exaggeration=12, learning_rate=200, random_state=42)
        # tsne = TSNE(n_components=2, perplexity=30, learning_rate=200, random_state=0)
        tsne_results4 = tsne.fit_transform(data4)

        # 可视化
        plt.figure(figsize=(6, 6))
        colors = ['#FFBCBB', '#B99CD4']
        for i, label in enumerate(np.unique(label4)):
            indices = np.where(label4 == label)
            plt.scatter(tsne_results4[indices, 0], tsne_results4[indices, 1], color=colors[i],
                        label=f'Class {label}')
        plt.gca().set_axis_off()
        ...
        plt.gca().xaxis.set_major_locator(plt.NullLocator())
        plt.gca().yaxis.set_major_locator(plt.NullLocator())
        plt.savefig('dom4.png', dpi=300, bbox_inches='tight', pad_inches=0)

        # 显示图表（注意，显示出来的图表可能会因为 Jupyter Notebook 或其他IDE的显示设置而包含边距）
        plt.show()
        # 5
        data5 = torch.cat(data5)
        data5 = data5.numpy()
        label5 = torch.cat(batch_label5)
        label5 = label5.numpy()

        tsne = TSNE(n_components=2, perplexity=30, early_exaggeration=12, learning_rate=200, random_state=42)
        # tsne = TSNE(n_components=2, perplexity=30, learning_rate=200, random_state=0)
        tsne_results5 = tsne.fit_transform(data5)

        # 可视化
        plt.figure(figsize=(6, 6))
        colors = ['#FFBCBB', '#B99CD4']
        for i, label in enumerate(np.unique(label5)):
            indices = np.where(label5 == label)
            plt.scatter(tsne_results5[indices, 0], tsne_results5[indices, 1], color=colors[i],
                        label=f'Class {label}')
        plt.gca().set_axis_off()
        ...
        plt.gca().xaxis.set_major_locator(plt.NullLocator())
        plt.gca().yaxis.set_major_locator(plt.NullLocator())
        plt.savefig('dom5.png', dpi=300, bbox_inches='tight', pad_inches=0)

        # 显示图表（注意，显示出来的图表可能会因为 Jupyter Notebook 或其他IDE的显示设置而包含边距）
        plt.show()
        # 6
        data6 = torch.cat(data6)
        data6 = data6.numpy()
        label6 = torch.cat(batch_label6)
        label6 = label6.numpy()

        tsne = TSNE(n_components=2, perplexity=30, early_exaggeration=12, learning_rate=200, random_state=42)
        # tsne = TSNE(n_components=2, perplexity=30, learning_rate=200, random_state=0)
        tsne_results6 = tsne.fit_transform(data6)

        # 可视化
        plt.figure(figsize=(6, 6))
        colors = ['#FFBCBB', '#B99CD4']
        for i, label in enumerate(np.unique(label6)):
            indices = np.where(label6 == label)
            plt.scatter(tsne_results6[indices, 0], tsne_results6[indices, 1], color=colors[i],
                        label=f'Class {label}')
        plt.gca().set_axis_off()
        ...
        plt.gca().xaxis.set_major_locator(plt.NullLocator())
        plt.gca().yaxis.set_major_locator(plt.NullLocator())
        plt.savefig('dom6.png', dpi=300, bbox_inches='tight', pad_inches=0)

        # 显示图表（注意，显示出来的图表可能会因为 Jupyter Notebook 或其他IDE的显示设置而包含边距）
        plt.show()
        # 7
        data7 = torch.cat(data7)
        data7 = data7.numpy()
        label7 = torch.cat(batch_label7)
        label7 = label7.numpy()

        tsne = TSNE(n_components=2, perplexity=30, early_exaggeration=12, learning_rate=200, random_state=42)
        # tsne = TSNE(n_components=2, perplexity=30, learning_rate=200, random_state=0)
        tsne_results7 = tsne.fit_transform(data7)

        # 可视化
        plt.figure(figsize=(6, 6))
        colors = ['#FFBCBB', '#B99CD4']
        for i, label in enumerate(np.unique(label7)):
            indices = np.where(label7 == label)
            plt.scatter(tsne_results7[indices, 0], tsne_results7[indices, 1], color=colors[i],
                        label=f'Class {label}')
        plt.gca().set_axis_off()
        ...
        plt.gca().xaxis.set_major_locator(plt.NullLocator())
        plt.gca().yaxis.set_major_locator(plt.NullLocator())
        plt.savefig('dom7.png', dpi=300, bbox_inches='tight', pad_inches=0)

        # 显示图表（注意，显示出来的图表可能会因为 Jupyter Notebook 或其他IDE的显示设置而包含边距）
        plt.show()
        # 8
        data8 = torch.cat(data8)
        data8 = data8.numpy()
        label8 = torch.cat(batch_label8)
        label8 = label8.numpy()

        tsne = TSNE(n_components=2, perplexity=30, early_exaggeration=12, learning_rate=200, random_state=42)
        # tsne = TSNE(n_components=2, perplexity=30, learning_rate=200, random_state=0)
        tsne_results8 = tsne.fit_transform(data8)

        # 可视化
        plt.figure(figsize=(6, 6))
        colors = ['#FFBCBB', '#B99CD4']
        for i, label in enumerate(np.unique(label8)):
            indices = np.where(label8 == label)
            plt.scatter(tsne_results8[indices, 0], tsne_results8[indices, 1], color=colors[i],
                        label=f'Class {label}')
        plt.gca().set_axis_off()
        ...
        plt.gca().xaxis.set_major_locator(plt.NullLocator())
        plt.gca().yaxis.set_major_locator(plt.NullLocator())
        plt.savefig('dom8.png', dpi=300, bbox_inches='tight', pad_inches=0)
