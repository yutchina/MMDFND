...
class MultiDomainPLEFENDModel(torch.nn.Module):
    def __init__(self,emb_dim,mlp_dims,bert,out_channels,dropout):
        ...
        for i in range(self.num_share):
            ...
            for j in range(self.num_expert):
                ...
                text_share.append(Block(dim=self.unified_dim, num_heads=8))
        ...
        self.image_model = image_extractor(out_channels)
    ...
    def forward(self,**kwargs):
        ...
        image_feature = self.image_model.forward_ying(image) #torch.Size([32, 197, 768])
        #image_feature = self.image_model(image)#([64, 320])
        text_atn_feature, _ = self.text_attention(text_feature)#([64, 768])
        image_atn_feature, _ = self.image_attention(image_feature)#([32, 768])
        #print("image_atn_feature", image_atn_feature.size())
        mm_atn_feature, _ = self.mm_attention(torch.cat((image_feature, text_feature), dim=1))##([32, 768])
        ...
        for i in range(self.domain_num):
            gate_expert = 0
            for j in range(self.num_expert):
                tmp_expert = self.text_experts[i][j](text_feature+self.positional_text)
            ...
            for j in range(self.num_share):
                ...
                #print("gate_expert",gate_expert.size())  ([16, 320])
        ...
        for i in range(self.domain_num):
            gate_expert = 0
            for j in range(self.num_expert):
                tmp_expert = self.text_experts[i][j](text_feature+self.positional_text)#([16, 197, 768])
                gate_expert += (tmp_expert*text_gate_out_list[i][:,j].unsqueeze(1).unsqueeze(1))##([16, 197, 768]*[16, 1,1])
            for j in range(self.num_share):
                tmp_expert = self.text_share_expert[0][j](text_feature)
                gate_expert += (tmp_expert*text_gate_out_list[i][:,j].unsqueeze(1).unsqueeze(1))
            gate_expert = gate_expert[:, 0]
            #print("gate_expert",gate_expert.size()) ([16,768])
            text_gate_expert_value.append(gate_expert)
        #image
        ...
        for i in range(self.domain_num):
            gate_expert = 0
            for j in range(self.num_expert):
                tmp_expert = self.image_experts[i][j](image_feature+self.positional_image)#([32, 197, 768])
                gate_expert += (tmp_expert*image_gate_out_list[i][:,j].unsqueeze(1).unsqueeze(1))##([32, 197, 768]*[32, 1, 1])
            for j in range(self.num_share):
                tmp_expert = self.image_share_expert[0][j](text_feature)
                gate_expert += (tmp_expert*image_gate_out_list[i][:,j].unsqueeze(1).unsqueeze(1))
            gate_expert = gate_expert[:, 0]
            #print(gate_expert.size()) (32,768)
        ...
        for i in range(self.domain_num):
            ...
            pre = torch.sigmoid(self.text_classifier_list[i](text_gate_expert_value[i]).squeeze(1))
            label_pred.append(pre)
        label_pred_list = []
        label_pred_avg = 0
        for i in range(self.domain_num):
