...
class MultiDomainPLEFENDModel(torch.nn.Module):
    def __init__(self, emb_dim, mlp_dims, bert, out_channels, dropout):
        ...
        self.clip_fusion0 = clip_fuion(768, 320, [348], 0.1)
    ...
    def forward(self, **kwargs):
        ...
        clip_fusion_feature = self.clip_fusion0(fusion_atn_feature)
        ...
        for epoch in range(self.epoches):
            ...
            if mark == 'save':
                torch.save(self.model.state_dict(),
                           os.path.join(self.save_param_dir, 'parameter_clip_ITA.pkl'))
            elif mark == 'esc':
                break
            else:
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_param_dir, 'parameter_clip_ITA.pkl')))
        results0,results1,results2,results3 = self.test(self.test_loader)

        return results0, os.path.join(self.save_param_dir, 'parameter_clip_ITA.pkl')
